...
static
#endif
struct cpufreq_governor cpufreq_gov_pegasusq_stock = {
	.name                   = "pegasusqstock",
...
struct workqueue_struct *dvfs_workqueue_stock;
...
static void apply_hotplug_lock(void)
{
	...
	queue_work_on(dbs_info->cpu, dvfs_workqueue_stock, work);
}

int cpufreq_pegasusq_cpu_lock_stock(int num_core)
{
	int prev_lock;
...
int cpufreq_pegasusq_cpu_unlock_stock(int num_core)
{
...
void cpufreq_pegasusq_min_cpu_lock_stock(unsigned int num_core)
{
	int online, flag;
	struct cpu_dbs_info_s *dbs_info;
	...
	if (flag <= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_workqueue_stock, &dbs_info->up_work);
}

void cpufreq_pegasusq_min_cpu_unlock_stock(void)
{
	...
	if (flag >= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_workqueue_stock, &dbs_info->down_work);
}
...
struct cpu_usage_history *hotplug_history_stock;
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_pegasusq_min_cpu_unlock_stock();
	else
		cpufreq_pegasusq_min_cpu_lock_stock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_pegasusq_cpu_unlock_stock(prev_lock);
	...
	ret = cpufreq_pegasusq_cpu_lock_stock(input);
	if (ret) {
...
{
	int num_hist = hotplug_history_stock->num_hist;
	struct cpu_usage *usage;
	int freq, rq_avg;
	int i;
	...
	for (i = num_hist - 1; i >= num_hist - up_rate; --i) {
		usage = &hotplug_history_stock->usage[i];
	...
	if (min_freq >= up_freq && min_rq_avg > up_rq) {
		...
		hotplug_history_stock->num_hist = 0;
		return 1;
	}
	return 0;
}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	hotplug_history_stock->usage[num_hist].freq = policy->cur;
	hotplug_history_stock->usage[num_hist].rq_avg = get_nr_run_avg();
	++hotplug_history_stock->num_hist;
	...
	for_each_cpu(j, policy->cpus) {
		...
		hotplug_history_stock->usage[num_hist].load[j] = load;
	...
	if (check_up()) {
		queue_work_on(this_dbs_info->cpu, dvfs_workqueue_stock,
			      &this_dbs_info->up_work);
	} else if (check_down()) {
		queue_work_on(this_dbs_info->cpu, dvfs_workqueue_stock,
			      &this_dbs_info->down_work);
	}
	if (hotplug_history_stock->num_hist  == max_hotplug_rate)
		hotplug_history_stock->num_hist = 0;
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, dvfs_workqueue_stock, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, dvfs_workqueue_stock,
...
unsigned int prev_freq_step_stock;
unsigned int prev_sampling_rate_stock;
static void cpufreq_pegasusq_early_suspend(struct early_suspend *h)
{
	...
	dbs_tuners_ins.early_suspend =
		atomic_read(&g_hotplug_lock);
#endif
	prev_freq_step_stock = dbs_tuners_ins.freq_step;
	prev_sampling_rate_stock = dbs_tuners_ins.sampling_rate;
...
static void cpufreq_pegasusq_late_resume(struct early_suspend *h)
{
	...
	dbs_tuners_ins.freq_step = prev_freq_step_stock;
	dbs_tuners_ins.sampling_rate = prev_sampling_rate_stock;
#if EARLYSUSPEND_HOTPLUGLOCK
	apply_hotplug_lock();
	start_rq_work();
#endif
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	hotplug_history_stock = kzalloc(sizeof(struct cpu_usage_history), GFP_KERNEL);
	if (!hotplug_history_stock) {
		pr_err("%s cannot create hotplug history array\n", __func__);
		ret = -ENOMEM;
		goto err_hist;
	}

	dvfs_workqueue_stock = create_workqueue("kpegasusq");
	if (!dvfs_workqueue_stock) {
		pr_err("%s cannot create workqueue\n", __func__);
		ret = -ENOMEM;
		goto err_queue;
	}

	ret = cpufreq_register_governor(&cpufreq_gov_pegasusq_stock);
	if (ret)
		goto err_reg;
...
err_reg:
	destroy_workqueue(dvfs_workqueue_stock);
err_queue:
	kfree(hotplug_history_stock);
err_hist:
	kfree(rq_data);
	return ret;
}
...
{
	cpufreq_unregister_governor(&cpufreq_gov_pegasusq_stock);
	destroy_workqueue(dvfs_workqueue_stock);
	kfree(hotplug_history_stock);
	kfree(rq_data);
}
