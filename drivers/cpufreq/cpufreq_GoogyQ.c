/*
 *  drivers/cpufreq/cpufreq_GoogyQ.c
 *
 *  Copyright (C)  2001 Russell King
 *            (C)  2003 Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>.
 *                      Jun Nakajima <jun.nakajima@intel.com>
 *  Modified by faux123 as intellidemand
 *  Modified by googyanas as GoogyQ (2014)
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_GOOGYQ
static
#endif
struct cpufreq_governor cpufreq_gov_GoogyQ = {
       .name                   = "GoogyQ",
...
static int GoogyQ_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void GoogyQ_powersave_bias_init_cpu(int cpu)
{
...
static void GoogyQ_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		GoogyQ_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_GoogyQ Governor Tunables */
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of GoogyQ, waiting for disk IO is an
				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		GoogyQ_powersave_bias_init_cpu(cpu);
		/*
		...
		if (!GoogyQ_powersave_bias_setspeed(
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (policy->max < this_dbs_info->cur_policy->cur)
			...
			GoogyQ_powersave_bias_setspeed(
				this_dbs_info->cur_policy,
				policy,
				dbs_tuners_ins.powersave_bias);
...
#ifdef CONFIG_EARLYSUSPEND
static void cpufreq_GoogyQ_early_suspend(struct early_suspend *h)
{
	mutex_lock(&dbs_mutex);
...
static void cpufreq_GoogyQ_late_resume(struct early_suspend *h)
{
	mutex_lock(&dbs_mutex);
...
static struct early_suspend cpufreq_GoogyQ_early_suspend_info = {
	.suspend = cpufreq_GoogyQ_early_suspend,
	.resume = cpufreq_GoogyQ_late_resume,
...
#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&cpufreq_GoogyQ_early_suspend_info);
#endif
	ret = cpufreq_register_governor(&cpufreq_gov_GoogyQ);
	if (ret)
		goto error_free;
...
{
	cpufreq_unregister_governor(&cpufreq_gov_GoogyQ);
#ifdef CONFIG_EARLYSUSPEND
	unregister_early_suspend(&cpufreq_GoogyQ_early_suspend_info);
#endif
...
MODULE_DESCRIPTION("'cpufreq_GoogyQ' - An intelligent dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_GOOGYQ
